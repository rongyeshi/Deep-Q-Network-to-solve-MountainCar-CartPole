...
from keras.layers import Conv2D, Dense, Flatten, Lambda, Add
...
class QNetwork():
    ...
    def __init__(self, environment_name):
        ...
        if environment_name=='CartPole-v0':
            self.learning_rate = 0.25e-3 # for CartPole lr = 0.25e-3, need 2020 episodes; for MountainCar
        else:
        ...
        stream_V = Dense(8, activation='relu')(net)
        stream_A = Dense(8, activation='relu')(net)
        
        value = Dense(1, activation='linear')(stream_V)
        advt = Dense(self.Num_output, activation='linear')(stream_A)
        
        #combine the two
        advt = Lambda(lambda advt: advt - K.mean(advt, axis=-1, keepdims=True))(advt)
        value = Lambda(lambda value: K.tile(value, [1, self.Num_output]))(value)
        
        
        self.outputs = Add()([value, advt])#output layer
...
class DQN_Agent():
        ...
        while True:
            ...
            while True:
                ...
                for i in list(range(0,len(minibatch))):
                    ...
                    #targets[i, action_t]= reward_t + int(terminal_t) * gamma * max(Q_sa)
                ...
                if self.environment_name=='CartPole-v0' and itera % int(self.iteration/3.0)==0:
                    ...
                    self.QNet.model.save_weights("save/"+self.environment_name+"DuRM_stab"+str(itera)+".h5")
            ...
            if self.environment_name == 'MountainCar-v0' and episode % int(self.episode/3.0)==0:
                    ...
                    self.QNet.model.save_weights("save/"+self.environment_name+"DuRM_stab"+str(episode)+".h5")

            
            if episode % 20==0:
                self.QNet.model.save_weights("save/"+self.environment_name+"DuRM_stab.h5")
                kk = self.test_in_train()
                if self.environment_name=='CartPole-v0':
                    if episode > 2000 and kk > 195 and self.QNet.learning_rate>0.00005:
                        print("Do decay!")
                        self.QNet.learning_rate = min(self.QNet.learning_rate/2.0, 0.00005)
    ...
    def test(self, model_file=None, numb=100):
        ...
        self.QNet.model.load_weights(model_file)
